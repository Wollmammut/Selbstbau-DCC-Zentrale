...
#define OUT_1    4  // Arduino pin for DCC signal HIGH 
                      // this pin is connected to "INA" of L298N motor driver
#define OUT_2    5  // Arduino pin for DCC signal LOQ 
                      // this pin is connected to "INB" of L298N motor driver
...
void setup(void) {
  ...
  pinMode(OUT_1,OUTPUT);   // this is for the DCC Signal
  
  pinMode(OUT_2,OUTPUT);   // will be kept high, PWM pin
  digitalWrite(OUT_2, 1);
...
boolean readSerial()
{
  ...
  {
    byte cmdBuffer[4];
    Serial.readBytes(cmdBuffer, CMD_LENGTH);
    switch (cmdBuffer[0])
    {
      case 0:
        handleSpeedCMD(cmdBuffer);
      break;
      case 1:
      break;
      default:
...
boolean handleSpeedCMD(byte cmdBuffer[])  
{
   boolean changed = false;
  
   locoSpeed = cmdBuffer[1] & 0b0111; // hacky way to limit speed from 0 to 127
